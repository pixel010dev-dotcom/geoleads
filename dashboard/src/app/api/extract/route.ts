...
// User Agent rotation - 20+ real user agents for better stealth
const USER_AGENTS = [
...
const startTime = Date.now();
  ...
  try {
    ...
    const context = await browser.newContext({
      userAgent: getRandomUserAgent(), // Random user agent for better stealth
      viewport: { width: 1920 + Math.floor(Math.random() * 100), height: 1080 + Math.floor(Math.random() * 100) }, // Slight variation
      locale: 'pt-BR',
      timezoneId: 'America/Sao_Paulo',
      geolocation: { latitude: -23.5505 + (Math.random() - 0.5) * 0.01, longitude: -46.6333 + (Math.random() - 0.5) * 0.01 }, // Slight variation
